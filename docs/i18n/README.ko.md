...
**NPX 사용자 분들께:** `npx`를 통해 설치한 모든 사용자에게 리버브와 배경 음악이 무음이었습니다. npm 패키지에서 추출된 훅 파일에 실행 비트(644)가 없어 `audio-processor.sh`가 코드 126으로 종료되었습니다. 호출 측의 `bash` 프리픽스와 postinstall의 `chmod 755` 단계로 수정되었습니다.

**보이스 브라우저:** 미리 듣기 버튼이 이제 설정한 리버브와 배경 음악을 적용합니다 — 이전에는 효과 없이 원시 오디오를 재생했습니다.

**MCP 도구:** `text_to_speech`가 이제 올바른 오디오 파일 경로를 반환합니다(이모지 쓰레기 없음), 보이스 이름이 응답에 포함됩니다.

**배경 음악 토글:** TUI에서 음악을 활성화하면 실제로 활성화됩니다 — bash 훅이 읽는 플래그 파일이 동기화 상태로 유지됩니다.

## 🌟 v5.6.8 신기능 — WSL 보이스 라우팅 수정 + 세션 라이프사이클 안정성
