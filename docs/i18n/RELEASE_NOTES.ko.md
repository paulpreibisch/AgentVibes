...
## 🎧 v5.14.0 — 제대로 되는 설치, 믿을 수 있는 미리 듣기

**출시일:** 2026-07-19 · 이제 `latest` 채널 — `npm install agentvibes@latest`

밀린 것을 한꺼번에 정리한 릴리스입니다. 5.13.1을 쓰고 계셨다면 5.13.2의 내용까지 모두 담겨 있습니다 — 그 버전은 태그만 붙었을 뿐 npm에는 올라가지 못했거든요.

### 📥 Mac과 Linux에서의 설치가 이제 진짜로 됩니다

이번 릴리스의 핵심입니다. 새로 설치한 Mac이나 Linux에서는 Piper 설치와 음성 다운로드가 **아예 실행조차 되지 않았습니다**. "Installing Piper TTS…"라는 메시지 뒤에 "installation failed or was cancelled"가 뜨면서, 시작한 적도 없는 단계를 탓하고 있었죠.

무려 51번의 릴리스 동안 고장 나 있었습니다. 저희가 개발에 쓰는 단 하나의 환경에서만 동작했고, 그래서 이토록 오래 아무도 눈치채지 못했습니다. 이제 수정되었고, 실제로 여러분이 설치하는 방식 그대로 테스트합니다.

여기에 얽힌 몇 가지 설정 문제도 함께 해결했습니다. 일부 스크립트가 Mac과 Linux에서는 읽을 수 없는 Windows 형식으로 저장되어 있어 아무것도 하지 못한 채 멈춰버렸습니다. 이제 올바른 형식으로 들어갑니다.

### 🔧 직접 고친 파일은 계속 여러분의 것입니다

AgentVibes가 설치하는 훅 스크립트를 손봐 두셨다면, 업데이트가 그 작업을 덮어쓸 수 있었습니다. Windows에서는 더 심각했는데, 업데이터가 25개 스크립트 중 8개만 알고 있어서 나머지는 잘못 처리되고 있었습니다.

이제 모든 스크립트가 빠짐없이 관리되며, 여러분이 수정한 파일은 손대기 전에 시각이 찍힌 백업으로 복사됩니다. `play-tts.ps1`을 고쳐 두셨다면, 원래 버전이 바로 옆에 이런 이름으로 저장됩니다:

```
play-tts.ps1.user.bak.20260719-143052
```

파일명의 시각은 해당 업데이트가 실행된 날짜와 시간입니다. 따라서 업데이트할 때마다 각각의 백업이 따로 남고, 맨 처음 것뿐 아니라 언제든 원하는 시점으로 되돌아갈 수 있습니다.

### 🎵 미리 듣기 버튼이 전체 믹스를 들려줍니다

설정 화면의 미리 듣기 버튼은 여러분이 고른 **음성**, 적용한 **리버브와 오디오 효과**, 그리고 **배경 음악**을 한꺼번에 재생해서 에이전트가 실제로 어떤 소리를 낼지 그대로 들려주는 기능입니다. 그런데 두 가지가 이걸 망가뜨리고 있었습니다.

Hermes 에이전트를 설정할 때는 음성과 효과만 재생되고 음악은 통째로 빠졌습니다. 그리고 Windows에서는 음악과 음성을 섞어 주는 도구인 **ffmpeg**에 접근할 수 없으면 음악이 조용히 사라졌습니다 — 음성은 멀쩡히 나오니 아무 문제가 없어 보였죠.

둘 다 수정되었습니다. 그리고 ffmpeg가 정말로 없을 때는, 이유를 짐작해야 하는 무음 대신 명확한 안내 메시지가 표시됩니다.

### 🎙️ 제대로 된 음성으로, 이름까지 알려주는 미리 듣기

이제 음성 미리 듣기는 전역 설정이 무엇이든 상관없이 그 음성 고유의 엔진을 사용하며, 지금 어떤 엔진으로 듣고 있는지도 알려줍니다. Windows와 Mac의 시스템 음성은 원격 미리 듣기에서도 동작합니다. Windows 음성 목록에는 실제로 선택 가능한 음성만 표시되므로, 골라도 말하지 않는 음성을 만날 일이 없습니다.

### 🟢 로컬인지 원격인지 한눈에

설정 화면에서 오디오 출력 대상이 색으로 구분됩니다 — **Local**은 초록색, **Remote**는 빨간색. 방이 왜 이렇게 조용한지 궁금할 때 유용합니다. 대개는 소리가 다른 컴퓨터로 가고 있다는 뜻이거든요.

### 🪟 Windows에서 다른 기기로 오디오 보내기

Windows에서 다른 컴퓨터로 오디오를 보낼 때 중간에 파일 경로가 망가지면서 배경 음악이 조용히 사라질 수 있었습니다. 수정되었습니다.

### 📦 더 깔끔해진 다운로드

패키지에서 애초에 필요 없던 파일들을 걷어냈습니다. 실려서는 안 될 로컬 설정 잔재도 포함해서요. 반대로 앱이 실행하라고 안내하는 것들은 이제 모두 실제로 포함됩니다.

### 🧰 내부 개선

테스트 스위트가 이제 Windows, Mac, Linux 모두에서 새로 받은 체크아웃 상태로 깨끗하게 통과합니다 — 그동안은 위의 설치 문제 같은 진짜 버그를 가려버리는 방식으로 실패하고 있었습니다. 이번에 고친 것들은 새 테스트로 못 박아 두었으니, 조용히 되살아날 수 없습니다. 백그라운드 프로세스가 종료되지 않고 멈춰 있던 문제도 함께 수정했습니다.
