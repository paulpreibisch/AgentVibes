...
## v5.15.1 — 미리 듣기가 어디서 재생되는지 알기

**출시일:** 2026-07-21 · `npm install agentvibes@latest`

우리가 계속 부딪혔던 문제가 있습니다. Space를 눌러 음성이나 음악 트랙을 미리 들을 때, 그것이 *내 컴퓨터에서* 재생될지 아니면 *원격 수신기로* 전송될지 알 방법이 없었습니다. 헤드리스 장비에서는 미리 듣기를 눌러도… 아무 소리도 들리지 않았고, 그 이유조차 알 수 없었습니다.

그래서 고쳤습니다. 이제 모든 미리 듣기는 소리가 어디로 가는지 정확히 알려줍니다 — **(locally)** 또는 **(remotely via SSH)** — 지금 확인하고 있는 바로 그 행에 표시됩니다. 더 이상 추측할 필요가 없습니다.

그리고 손을 댄 김에 **앱 전체에서 미리 듣기를 표준화**했습니다. 음성 선택기(Kokoro, Piper, ElevenLabs, 그리고 에이전트별 BMAD 음성)와 Music 페이지가 이제 모두 동일한 `Previewing (locally / remotely via SSH)` 표시를 보여주므로, 어디서나 똑같은 모습과 동작을 합니다. 작아 보이지만 실제로 무척 유용한 변화입니다.

한 가지 실질적인 이점이 있습니다. 프로젝트가 TTS를 원격 수신기로 라우팅한다면, **이제 음악 미리 듣기도 이를 따릅니다** — 이전에는 음악이 조용히 로컬 컴퓨터에서 재생되었는데, 헤드리스 서버에서는 곧 무음을 의미했습니다.

### 이번 릴리스의 그 밖의 변경 사항

- **Agents 탭이 실제 BMAD 에이전트를 보여줍니다** (Mary, Winston, Amelia, John, Paige, Sally…) — 스킬의 내부 헬퍼가 아니라요. 그리고 탭으로 다시 전환할 때 스스로 재확인하므로, 새로 설치한 BMAD가 재시작 없이도 나타납니다.
- **Reset이 더 이상 Receiver 탭으로 끌고 가지 않습니다.** 이전에는 전역적으로 Receiver로 이동하는 것과 동일한 `X` 키에 매핑되어 있었으나, 이제는 `Del`입니다.

## v5.15.0 — Windows에서의 다중 세션 제어

**출시일:** 2026-07-20 · `npm install agentvibes@latest`

여러 에이전트 세션을 동시에 실행하신다면, 이번 릴리스는 macOS와 Linux가 5.13.0에서 얻었던 것과 동일한 제어 기능을 Windows에도 제공합니다. 세션은 켜기 전까지 조용히 있으며, 각 세션은 어느 창이 말하고 있는지 알려줄 수 있습니다.

### 세션이 더 이상 한꺼번에 말하지 않습니다

여러 프로젝트에서 AgentVibes를 실행하면 이전에는 열려 있는 모든 세션이 스스로를 알렸고, 하나만 말하도록 유지할 마땅한 방법이 없었습니다. macOS와 Linux에서는 5.13.0에서 이 문제가 해결되었습니다. **하지만 Windows에서는 그렇지 못했습니다** — Windows 세션 훅은 설정과 관계없이 모든 세션에서 음성을 활성화했습니다.

이제 Windows도 동일한 규칙을 따릅니다. 세션은 사용자가 명시적으로 활성화한 프로젝트에서만 말합니다. 활성화하지 않은 프로젝트는 **아무것도 기여하지 않습니다** — 무음 오디오조차 아니라, 추가되는 지시도 없고 토큰 비용도 없습니다.

### Windows에서 음소거가 작동합니다

`/agent-vibes:mute`는 Windows 오디오에 아무런 효과가 없었습니다. Windows 플레이어가 명령이 기록하는 설정과 다른 설정을 읽었기 때문에, macOS와 Linux에서만 음소거되었습니다.

이제 Windows에서도 프로젝트 음소거와 전역 스위치가 모두 반영됩니다:

- `/agent-vibes:mute` — 이 프로젝트를 조용히 하기
- `/agent-vibes:unmute` — 다른 모든 것이 전역적으로 음소거된 상태에서도 이 프로젝트를 다시 켜기

이로써 "어디서나 끄고, 지금 작업 중인 하나의 프로젝트에서만 켜기"가 모든 플랫폼에서 명령 한 쌍으로 가능해졌습니다.

### 세션이 Windows에서 스스로를 소개할 수 있습니다

프리텍스트에 `{{session}}`을 포함하면 세션이 한 번 스스로를 소개합니다:

```
Claude on my-app in Windows Terminal
```

프로젝트 이름을 밝히고 터미널을 감지합니다 — Windows Terminal, VS Code, Ghostty, iTerm, Terminal, WezTerm, tmux 등. 모든 줄 앞이 아니라 세션당 한 번만 알립니다. 이전에는 macOS와 Linux에서만 작동했습니다.

### 모든 플랫폼에 적용되는 관련 수정

그 소개를 생성하는 스크립트가 업데이트 과정에서 누락되어, 전역 설치에서는 전혀 전달되지 않았고 이 기능이 조용히 아무 일도 하지 않았습니다. 이제 macOS, Linux, Windows에 모두 포함됩니다.

### 업그레이드

기존 선택은 보존됩니다 — 이미 음소거하거나, 음소거를 해제하거나, 프로젝트를 활성화하셨다면 업데이트는 그 설정을 그대로 둡니다.

**전역 설치를 사용하는 Windows 사용자가 알아둘 한 가지 변경 사항:** 이제 세션은 활성화하기 전까지 꺼져 있으므로, 전역 Windows 설치는 업데이트 후 조용해집니다. `/agent-vibes:unmute`로 원하는 프로젝트를 켜세요. 프로젝트 설치는 자동으로 활성화되며 영향을 받지 않습니다.

### 품질

새로운 회귀 테스트는 세션 게이트, 음소거 규칙, 자기소개가 두 런타임에서 동일하게 동작하는지 검증합니다. 따라서 빌드 실패 없이는 플랫폼이 다시 어긋날 수 없습니다.

---

## v5.14.0 — 안정적인 설치와 완전한 오디오 미리 듣기

**출시일:** 2026-07-19 · `npm install agentvibes@latest`

이번 릴리스는 두 가지에 집중했습니다. 모든 플랫폼에서 AgentVibes가 올바르게 설치되도록 하는 것, 그리고 미리 듣기 버튼이 에이전트의 실제 음성을 정확하게 들려주도록 하는 것입니다. 태그는 생성되었으나 npm에 게시되지 않았던 5.13.2의 모든 변경 사항도 함께 포함되어 있습니다.

### macOS와 Linux에서 설치가 정상적으로 완료됩니다

새로 설정한 macOS 또는 Linux 환경에서 Piper 설치와 음성 다운로드가 올바르게 실행됩니다. 이전에는 이 단계들이 실제로 실행되지 않은 채 실패를 보고하여, 신규 사용자가 동작하는 음성 엔진 없이 다음 단계를 찾지 못하는 상황이 발생했습니다. 첫 설치에서만 나타나던 오래된 문제였으므로, 이전에 설치를 중단하셨다면 다시 시도해 보실 만합니다.

관련하여, 여러 설치 스크립트가 올바른 줄 바꿈 형식으로 저장되어 macOS와 Linux에서 정상적으로 실행됩니다.

### 미리 듣기가 완전한 오디오 믹스를 재생합니다

미리 듣기 버튼은 이제 에이전트의 실제 음성을 그대로 재생합니다. 선택한 **음성**, 적용된 **리버브 및 오디오 효과**, 그리고 **배경 음악**이 함께 믹싱되어 들립니다.

이전에는 두 가지 경우에 불완전한 믹스가 재생되었습니다. Hermes 에이전트 미리 듣기에서는 배경 음악이 완전히 빠졌고, Windows에서는 음악과 음성을 믹싱하는 구성 요소인 ffmpeg를 사용할 수 없을 때 음악 트랙이 누락되었습니다. 두 문제 모두 해결되었으며, ffmpeg가 없는 경우에는 오디오가 조용히 축소되는 대신 원인을 명확히 알려주는 메시지가 표시됩니다.

### 사용자가 수정한 내용이 업데이트 후에도 유지됩니다

AgentVibes가 설치하는 훅 스크립트를 수정하셨다면, 업데이트 시 그 작업이 보존됩니다. 파일이 교체되기 전에 사용자의 버전이 타임스탬프가 붙은 백업으로 복사됩니다:

```
play-tts.ps1.user.bak.20260719-143052
```

업데이트마다 별도의 백업이 생성되므로 이전 버전도 계속 복구할 수 있습니다. Windows에서는 업데이트 적용 범위가 8개 스크립트에서 전체 25개로 확대되었습니다.

### 오디오 출력 대상을 한눈에 확인할 수 있습니다

설정 화면이 오디오가 재생되는 위치를 색상으로 구분합니다. **Local**은 초록색, **Remote**는 빨간색으로 표시됩니다. 오디오가 다른 컴퓨터로 전달되고 있을 때 이를 즉시 알아볼 수 있습니다.

### 음성 미리 듣기가 올바른 엔진을 사용합니다

음성 미리 듣기는 이제 전역 설정이 아니라 해당 음성 고유의 엔진을 사용하며, 어떤 엔진으로 재생되는지 알려줍니다. Windows와 macOS 시스템 음성이 원격 미리 듣기에서도 정상적으로 동작하고, Windows 음성 목록에는 실제로 선택할 수 있는 음성만 표시됩니다.

### Windows 원격 오디오

Windows에서 다른 컴퓨터로 오디오를 전송할 때 파일 경로가 전송 중에 변경되지 않습니다. 이전에는 이 문제로 배경 음악이 수신 측에 도달하지 못했습니다.

### 패키지와 품질

게시되는 패키지에서 불필요한 파일이 제외되었고, 애플리케이션이 참조하는 파일은 모두 포함되었습니다. 전체 테스트 스위트가 클린 체크아웃 상태에서 Windows, macOS, Linux 모두 통과하며, 위에서 설명한 미리 듣기 동작을 검증하는 회귀 테스트가 추가되었습니다.

---

## 🔧 v5.13.2 — 더 깔끔해진 설치, 더 편해진 설정

**출시일:** 2026-07-17 · 이제 `latest` 채널 — `npm install agentvibes@latest`

### 🎛️ 기본값으로 시작해서, 나만의 설정으로 만들어 갑니다

새로 설치하면 이제 음성, 배경 음악, 퍼소낼리티에 대한 내장 기본값으로 깨끗하게 시작합니다 — 그래서 처음 실행하는 순간부터 여러분만의 설정이 됩니다.

### 🐧 Mac과 Linux 설치가 제대로 작동합니다

설치를 준비하는 스크립트 몇 개가 Mac과 Linux가 읽을 수 없는 Windows 형식으로 저장되어 있어서, 아무것도 하지 못한 채 멈춰버리곤 했습니다. 이제는 올바른 형식으로 되어 있습니다. 새로 설치한 Mac이나 Linux 컴퓨터에서도 Piper 설치와 음성 다운로드가 다시 잘 작동합니다.

### 🔊 여러분이 고른 음성이 그대로 유지됩니다

어떤 음성이 어떤 엔진과 짝을 이루는지 기억하는 설정 파일이 살짝 잘못 읽히는 바람에, 여러분이 고른 엔진이 조용히 무시되고 있었습니다. 수정했습니다 — 이제 고른 대로 나옵니다.

### 📦 더 작고 더 깔끔해진 다운로드

패키지가 더 이상 필요 없던 파일들을 짊어지고 다니지 않습니다. 앱이 실행하라고 안내하는 것들도 이제 실제로 다 들어 있습니다.
...
### 🔒 보이지 않는 곳의 보안도 다듬었습니다

AgentVibes가 설정 파일을 읽을 때 사용하는 구성 요소 중 하나를 업데이트했습니다. 특별히 조작된 설정 파일 때문에 멈춰버릴 수 있는 문제가 있었습니다. 무언가가 도난당하거나 몰래 들여다볼 수 있었던 적은 한 번도 없었지만, 이제는 멈추는 일도 없어졌습니다. 여러분이 따로 하실 일은 없습니다 — 이미 반영되어 있습니다.
