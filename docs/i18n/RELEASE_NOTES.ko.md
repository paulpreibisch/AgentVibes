...
## 🔧 v5.13.1 — 이제 진짜로 업데이트되는 Windows 업데이트

**출시일:** 2026-07-16 · 이제 `latest` 채널 — `npm install agentvibes@latest`

### 🪟 여러분의 Windows 스크립트가 이제 정말로 업데이트됩니다

Windows에서는 에이전트가 말할 수 있게 해주는 작은 스크립트들이 `.claude/hooks` 폴더 안에 있습니다. 업데이트를 하면 이 스크립트들도 새로 고쳐진다고 했지만, Windows에서는 조용히 그렇게 되지 않아서 처음 설치했던 버전 그대로 몇 달씩 남아있을 수 있었습니다.

이제는 진짜로 업데이트됩니다. `npx agentvibes update`를 실행하면 놓치고 있던 모든 수정 사항을 받을 수 있습니다. 여러분이 직접 수정한 내용은 이전과 마찬가지로 `.user.bak` 파일로 안전하게 그 옆에 보관됩니다.

macOS나 Linux를 쓰고 있다면 달라지는 건 없습니다 — 그쪽에서는 이미 업데이트가 잘 작동하고 있었으니까요.
