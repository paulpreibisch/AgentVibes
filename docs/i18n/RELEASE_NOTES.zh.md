...
## 🎧 v5.14.0 — 装得上的安装，靠得住的预览

**发布日期：** 2026-07-19 · 现已进入 `latest` — `npm install agentvibes@latest`

这是一次分量十足的补课版本。如果你还停留在 5.13.1，这一版把 5.13.2 的全部内容也一并带上了 —— 那个版本虽然打了标签，却从未真正发布到 npm。

### 📥 Mac 和 Linux 上的安装，这次是真的能用了

这是本次最重要的修复。在全新的 Mac 或 Linux 上安装时，安装 Piper 和下载语音这两步**从头到尾根本没有执行过**。你会看到"正在安装 Piper TTS…"，紧接着就是"安装失败或已取消"—— 它在为一个压根没启动的步骤报错。

这个问题已经持续了 51 个版本。它一直以来只在我们自己开发用的那一套环境上能跑通，这也正是它长期无人察觉的原因。现在已经修复，并且是按照你真实的安装方式来测试的。

顺带解决了几个相关的安装问题：有些脚本此前以 Mac 和 Linux 读不懂的 Windows 格式保存，于是它们什么都没做就停住了。现在格式已经修正。

### 🔧 你改过的文件，依然属于你

如果你曾经修改过 AgentVibes 安装的任何 hook 脚本，更新时可能会把你的心血覆盖掉。在 Windows 上情况更糟 —— 更新程序只认得 25 个脚本中的 8 个，绝大多数都被错误地处理了。

现在每一个脚本都在覆盖范围内，凡是你改动过的内容，在被动到之前都会先复制成一份带时间戳的备份。假如你改过 `play-tts.ps1`，你会在它旁边看到自己的版本被保存为：

```
play-tts.ps1.user.bak.20260719-143052
```

时间戳就是那次更新的日期和时间，所以你每运行一次更新，都会留下属于它自己的备份 —— 你随时可以回到任意一个版本，而不只是最早的那一份。

### 🎵 预览按钮播放的是完整的混音

设置界面里的预览按钮，本该把所有东西合在一起播放 —— 你选的**语音**、任何**混响或音频效果**，以及你的**背景音乐** —— 好让你听到代理真正说话时的样子。但有两个问题破坏了它。

在配置 Hermes 代理时，预览会播放语音和效果，却把音乐完全落下了。而在 Windows 上，如果**ffmpeg**（负责把音乐和语音混合在一起的工具）无法访问，音乐就会被悄无声息地丢掉 —— 语音播放得好好的，表面上看不出任何异常。

两个问题都已修复。而当 ffmpeg 确实缺失时，你现在会收到一条明确的提示，而不是一片让你去猜的沉默。

### 🎙️ 用正确的语音预览，并且告诉你它叫什么

预览某个语音时，现在会使用该语音自己的引擎，而不是全局设定的那个，并且会告诉你正在听的是哪个引擎。Windows 和 Mac 的系统语音在远程预览中也能正常工作。Windows 语音列表现在只显示真正可选的语音 —— 不会再选到一个永远不出声的语音了。

### 🟢 本地还是远程，一眼看清

设置界面现在会给你的音频目的地上色：**Local** 为绿色，**Remote** 为红色。当你纳闷房间里怎么这么安静时，这一眼往往就能给出答案 —— 声音跑到另一台机器上去了。

### 🪟 Windows 把音频送往别处时

从 Windows 机器把音频发送到另一台电脑时，中途可能会把文件路径弄乱，从而悄悄丢掉你的背景音乐。已修复。

### 📦 更干净的安装包

安装包不再携带那些从来用不上的文件，包括一些本不该被打包进来的本地设置残留。应用提示你运行的每一样东西，现在都确实包含在内了。

### 🧰 引擎盖之下

测试套件现在能在 Windows、Mac 和 Linux 上从全新检出干净地跑通 —— 此前它的失败方式恰恰掩盖了像上面那个安装问题一样的真实 bug。新增的测试把这次的修复牢牢锁住，让它们无法悄悄卷土重来。另外还修复了一个后台进程可能挂起而不退出的问题。
