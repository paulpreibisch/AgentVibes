...
**发布日期：** 2026-07-21 · `npm install agentvibes@latest`

我们一直遇到这样一个问题：当你按下 Space 预览某个语音或音乐曲目时，你无从得知它究竟会*在你自己的机器上*播放，还是被发送*到你的远程接收器*。在无显示器的机器上，这意味着按下预览却听到……什么都没有 —— 而且完全不知道为什么。

于是我们修复了它。现在每次预览都会明确告诉你声音将去往何处 —— **(locally)** 或 **(remotely via SSH)** —— 就显示在你正在试听的那一行上。不用再猜了。

趁着这次改动，我们还**在整个应用中统一了预览体验**。语音选择器（Kokoro、Piper、ElevenLabs 以及各代理的 BMAD 语音）和 Music 页面现在都会显示相同的 `Previewing (locally / remotely via SSH)` 指示，让它在各处的外观和行为都完全一致。这是件小事，但事实证明它真的很有用。

一个实际的好处：如果某个项目将其 TTS 路由到远程接收器，**音乐预览现在也会随之路由** —— 此前它们会悄悄地在本地机器上播放，而在无显示器的服务器上这就等于是无声。

### 本次发布还包括

- **Agents 标签页现在显示你真实的 BMAD 代理**（Mary、Winston、Amelia、John、Paige、Sally……），而不是某个技能的内部辅助项 —— 而且当你切换回该标签页时它会重新检查，因此全新安装的 BMAD 无需重启即可显示出来。
- **Reset 不再把你猛地拉到 Receiver 标签页。** 它此前被映射到 `X`，也就是全局跳转到 Receiver 的同一个按键；现在它是 `Del`。

## v5.15.0 — Windows 上的多会话控制

**发布日期：** 2026-07-20 · `npm install agentvibes@latest`

如果你同时运行多个代理会话，本次发布让 Windows 获得了 macOS 和 Linux 在 5.13.0 中就已拥有的相同控制能力：会话默认保持静音，除非你将其开启，并且每个会话都能告诉你正在发声的是哪个窗口。

### 会话不再同时全部发声

此前在多个项目中运行 AgentVibes 意味着每个打开的会话都会自我播报，而没有简便的方式只让其中一个继续发声。在 macOS 和 Linux 上，这个问题已在 5.13.0 中得到解决。**但在 Windows 上却没有** —— Windows 会话 hook 无论你的设置如何，都会在每个会话中启用语音。

Windows 现在遵循相同的规则：会话只在你已明确启用的项目中发声。你尚未启用的项目**不会产生任何影响** —— 不是播放无声音频，而是不添加任何指令，也不产生 token 成本。

### 静音现已在 Windows 上生效

`/agent-vibes:mute` 此前对 Windows 音频毫无作用。它仅能让 macOS 和 Linux 静音，因为 Windows 播放器读取的设置与该命令写入的设置不同。

项目级静音和全局开关现已在 Windows 上都得到遵守：

- `/agent-vibes:mute` —— 让当前项目静音
- `/agent-vibes:unmute` —— 重新开启当前项目，即使其他一切都已全局静音

这使得"在所有地方关闭，只在我正在处理的那一个项目中开启"在每个平台上都只需一对命令即可实现。

### 会话可以在 Windows 上标识自己

在你的前缀文本中加入 `{{session}}`，会话便会自我介绍一次：

```
Claude on my-app in Windows Terminal
```

它会指明项目名称并检测终端 —— Windows Terminal、VS Code、Ghostty、iTerm、Terminal、WezTerm、tmux 等等。每个会话仅播报一次，而非在每一行之前都播报。此功能此前仅在 macOS 和 Linux 上有效。

### 一项惠及所有平台的相关修复

生成上述自我介绍的脚本此前缺失于更新过程之中，因此在全局安装中它从未被交付，该功能会悄无声息地什么也不做。现在它已同样包含在 macOS、Linux 和 Windows 中。

### 升级说明

你现有的选择将被保留 —— 如果你此前已经静音、取消静音或启用了某个项目，更新时该设置将保持不变。

**对于使用全局安装的 Windows 用户，有一处值得注意的变化：** 由于会话现在默认关闭，除非启用，因此全局 Windows 安装在更新后将保持静音。请使用 `/agent-vibes:unmute` 开启你想要的项目。项目级安装会自动启用，不受影响。

### 质量保障

新增的回归测试断言会话门控、静音规则以及自我标识在两种运行时上的行为完全一致，从而确保各平台不会在构建不失败的情况下再次出现分歧。
