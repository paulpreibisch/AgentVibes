...
- **每次预览都会显示它在哪里播放** — 语音和音乐预览现在会在你正在试听的那一行直接显示 **(locally)** 或 **(remotely via SSH)**，让你不用再猜（也不会在无显示器的机器上只听到一片寂静）。
- **整个应用中的预览已统一** — 语音选择器（Kokoro、Piper、ElevenLabs、各代理的 BMAD）和 Music 页面都会显示相同的指示；音乐预览现在也会随项目的远程接收器一同路由。
- **更清爽的 Agents 标签页** — 它会列出你真实的 BMAD 代理（而不是某个技能的内部辅助项），并在获得焦点时重新检查；**Reset** 已从 `X`（会跳转到 Receiver 标签页）移到 **`Del`**。

### v5.15.0 — Windows 上的多会话控制

- **会话默认保持静音，除非你启用它们** — 会话只在你已开启的项目中发声；其他会话不会添加任何指令，也不产生 token 成本。
- **`/agent-vibes:mute` 现已在 Windows 上生效** — 此前在该平台上完全无效。项目级和全局静音现已在每个平台上都得到遵守。
- **会话可以在 Windows 上自我介绍** — `{{session}}` 会播报 "Claude on my-app in Windows Terminal"，每个会话仅播报一次。
- **自我介绍现已送达全局安装** — 支撑该功能的脚本此前从未被更新程序在任何平台上交付。
- **全局 Windows 安装注意事项：** 本次更新后会话默认关闭 — 请使用 `/agent-vibes:unmute` 启用。

### v5.14.0 — 可靠的安装体验与完整的音频预览

包含 5.13.2 的全部改动，该版本未发布到 npm。

- **macOS 和 Linux 上的安装顺利完成** — 在全新机器上，安装 Piper 和下载语音现在都能正确执行，解决了一个长期存在的首次安装问题。
- **预览播放完整的混音** — 语音、混响/音效与背景音乐一同播放，让预览真实还原代理实际发声的效果。
- **更新保留你的自定义修改** — 在任何文件被替换之前，你修改过的 hook 脚本都会生成带时间戳的备份。
- **更清晰的音频输出位置** — 设置界面以绿色显示 **Local**、红色显示 **Remote**。
- **正确的预览引擎** — 预览使用所选语音自身的引擎并标明该引擎；Windows 和 macOS 语音在远程预览中可正常工作。

### v5.13.0 — 你的语音，随处可用

选用 **Windows** 或 **Mac** 内置的语音，无论你在何处聆听都能听到它们 — 即使你的代理运行在另一台电脑上。此外还有一声友好的提前提醒提示音，让你总能知道声音即将到来。

- **🖥️ 随时随地使用你电脑自带的语音** — 选择 Windows 或 Mac 语音，并在你的机器上听到它们；每个语音都会显示出来，不可用的会被清晰标记。
- **🗂️ 所有语音尽在一份列表** — Piper、Kokoro、ElevenLabs、Windows、Mac 和 Soprano 集中在同一处，所以你所看到的就是你能使用的。
- **🔔 提前提醒提示音** — 在语音或音乐预览之前会先播放一声短促的声音，让你知道音频即将到来。
- **🆔 会自我介绍的代理** — 可选的自我介绍，让你在团队中知道正在发声的是谁。

### v5.12.0 — 更强大的核心

在体验 Anthropic 全新 **Fable** 模型的抢先版一周期间，我们把 AgentVibes 的核心重建为**一个共享核心**。过去被复制到四个脚本中（并逐渐产生分歧）的语音 / 引擎 / 路由 / 音量 / 静音逻辑，如今集中在同一个地方 — 更简单、更一致、更稳定。

- **🔊 预览在正确的位置播放** — 配置了 SSH 远程后，语音和**音乐预览**会在你的接收器上播放；否则会在本地播放。
- **🧠 一个共享核心** — Kokoro 在 Linux 上的静默问题以及各语音之间的分歧已从源头修复，并在需要时提供安全回退。
- **🧹 移除了多余的 Voices 标签页** — 在 Setup 中为任何提供商选择语音。

### v5.11.0 — 神经网络语音

- **🧠 Kokoro** — 在你的 **CPU 上运行的本地神经网络 TTS，无需 GPU**（内置中文、日语、韩语）。
- **☁️ ElevenLabs** — 高级云端神经网络语音。
- 可组合的音频效果：在任意语音上叠加**混响**、**回声**和**合唱**。

## v5.7.7 — 派对模式语音恢复 + 改进

**派对模式代理现在再次发声：** BMAD `/party-mode` 现在可靠地调用正确的 AgentVibes 技能，每个代理的响应以其独特的声音大声朗读，带有每代理的音乐、预文本和混响 — 从 `~/.agentvibes/bmad-voice-map.json` 自动加载。

**新捆绑曲目：** 🌌 CelestialVelvet 已添加到内置目录。

**TUI 对比度修复：** 语音和代理选项卡中的选定行不再显示难以阅读的灰色文本。

**SSH 远程：** 修复了 `play-tts-ssh-remote.sh` 中的 "wait: pid is not a child of this shell" 错误。

## v5.7.6 新功能 — SSH 远程负载完整性 + 接收器重写

**SSH 远程音乐/语音修复：** 正确的项目音乐曲目和语音现在可以到达远程接收器 — 以前使用的是全局配置而非活动项目的设置。

**Bash 接收器重写：** Linux/Termux `agentvibes-receiver.sh` 已完全重写以解码当前的 base64 JSON 负载格式。v5.5 之前的旧位置参数格式已消失。

**不再有双重介绍：** 个性 pretext（如 "Bcs latin dance here"）不再通过 SSH 远程被说两次。`play-tts.sh` 将其添加到文本前面；接收器不再收到单独的 pretext 字段来再次添加。

**TUI 中显示 SSH 主机：** 设置和语音选项卡现在显示配置的 SSH 远程主机别名。

**安全修复**以及涵盖完整发送方 → 接收器往返的 24 个新 BATS 测试。
