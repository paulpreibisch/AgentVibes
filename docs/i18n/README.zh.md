...
## 🆕 装得上的安装，靠得住的预览（v5.14.0）

一次补课版本，把从未发布到 npm 的 5.13.2 的全部内容也一并带上了。

- **📥 Mac 和 Linux 上的安装真的能用了** — 在全新安装中，安装 Piper 和下载语音这两步从未执行过，却为一个压根没启动的步骤报错。这个问题持续了 51 个版本；现已修复，并经过完整测试。
- **🔧 你改过的文件依然属于你** — 更新不会再覆盖你修改过的 hook 脚本；你的版本会先被保存在它旁边，例如 `play-tts.ps1.user.bak.20260719-143052`。
- **🎵 预览播放完整的混音** — 预览按钮会把你的语音、混响/音效**以及**背景音乐一起播放；音乐不会再莫名消失，而当 ffmpeg 不可用时，你会收到明确提示，而不是一片沉默。
- **🎙️ 正确的语音，还告诉你它的名字** — 预览会使用被预览语音自己的引擎，并说明是哪一个；Windows 和 Mac 语音在远程预览中同样可用。
- **🟢 本地还是远程，一眼看清** — 音频目的地会以绿色显示 **Local**、红色显示 **Remote**。
