...
## 🌟 v5.8.0 新功能 — Soprano 现已正常工作 + 修复了所有引擎的语音选择器

**Soprano TTS 现在真正可以工作了：** Soprano（我们的神经网络 TTS 引擎）自发布以来一直在 Windows 上悄无声息地损坏 — 二进制文件名错误、PATH 被截断、wav 路径发送到错误的输出流、WebUI 服务器没有自动启动。一切都已修复。使用 `pip install soprano-tts` 安装，在设置标签页中选择 Soprano，AgentVibes 会处理其余的事情。

**语音选择器现在适用于 Windows SAPI 和 macOS Say：** 以前即使选择了 SAPI 或 macOS Say，选择器仍会显示整个 Piper 语音目录，空格键预览也通过错误的引擎播放。选择器现在为每个原生引擎显示恰好一个条目，并通过正确的二进制文件进行预览。

**自动保存不再破坏您的引擎设置：** 保存 LLM 配置不再悄悄将您选择的引擎覆盖回 Piper。
