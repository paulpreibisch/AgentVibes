...
# ---------------------------------------------------------------------------
# Protocol detection
# New protocol: play-tts-agentvibes-receiver-for-voiceless-connections.sh sends
#   SSH_ORIGINAL_COMMAND = "bash ~/.agentvibes/play-remote.sh 'B64_TEXT' 'VOICE' 'B64_AGENT'"
# Old protocol: SSH_ORIGINAL_COMMAND = raw base64-encoded JSON payload
# ---------------------------------------------------------------------------
$decoded = $null
if ($EncodedPayload -match "(?:play-remote\.sh|agentvibes-play\.sh)\s+'([A-Za-z0-9+/=]+)'\s+'([^']+)'(?:\s+'([A-Za-z0-9+/=]*)')?") {
    try {
        $decodedText  = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Matches[1]))
        $voiceArg     = $Matches[2]
        $decodedAgent = if ($Matches[3]) {
            [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Matches[3]))
        } else { 'agentvibes' }
        $decoded = @{ text = $decodedText; voice = $voiceArg; project = $decodedAgent } | ConvertTo-Json -Compress
    } catch {
        Write-Output "Error: Failed to decode agentvibes-receiver args: $_"
        exit 1
    }
}

if ($null -eq $decoded) {
    # Old protocol: validate and decode raw base64 JSON
